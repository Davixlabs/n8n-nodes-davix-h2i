...
- The HTTP execution path calls `this.helpers.httpRequest(requestOptions)` without timeout-specific fields.  
...
- No retry/maxRetries/backoff request options are set when building request options.  
  **Evidence:** `nodes/DavixH2I/GenericFunctions.ts:L39-L52`
- Request errors are caught and rethrown as `NodeApiError`; there is no retry loop around `this.helpers.httpRequest`.  
