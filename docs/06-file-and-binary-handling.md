...
The download helper uses `this.helpers.request` with `encoding: null` and `resolveWithFullResponse: true`, then returns `res.body as Buffer` and `content-type` for binary preparation. **Evidence:** `nodes/DavixH2I/GenericFunctions.ts:L165-L175`
...
[formData construction: images/files/watermarkImage]
    ↓
[davixRequest() using this.helpers.request()]
    ↓
...
- The node uses `this.helpers.request` for both API and file download paths; behavior details such as HTTP agent reuse, streaming internals, and memory management below n8n helper level are not confirmed in this repository. Not confirmed in code.
...
- `nodes/DavixH2I/GenericFunctions.ts` — HTTP helper usage (`this.helpers.request`), request wrapper, and binary download helper returning `Buffer` + MIME type.
