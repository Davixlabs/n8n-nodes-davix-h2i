...
- All network calls route through `davixRequest()`, which validates credentials/base URL and injects `x-api-key`. Request debug path is: credential retrieval → `validateBaseUrl()` → `buildDavixRequestOptions()` → `helpers.request()`. **Evidence:** `nodes/DavixH2I/GenericFunctions.ts:L18-L37`, `nodes/DavixH2I/GenericFunctions.ts:L39-L52`, `nodes/DavixH2I/GenericFunctions.ts:L106-L117`
...
- Downloads use `downloadToBinary()` with `resolveWithFullResponse: true`, then `prepareBinaryData(...)` for output binary fields. **Evidence:** `nodes/DavixH2I/GenericFunctions.ts:L165-L175`, `nodes/DavixH2I/DavixH2I.node.ts:L1362-L1368`, `nodes/DavixH2I/DavixH2I.node.ts:L1637-L1642`, `nodes/DavixH2I/DavixH2I.node.ts:L1787-L1792`
...
- Maintainer/author in package metadata: `Davixlabs`. **Evidence:** `package.json:L21-L23`
...
- README includes support/contact references (website, GitHub project, issue filing guidance) and contribution steps. **Evidence:** `README.md:L254-L263`, `README.md:L298-L303`
...
- `tests/GenericFunctions.test.js` — test coverage confirming request ID envelope parsing and auth header injection.
