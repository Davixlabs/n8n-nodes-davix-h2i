		...
		properties: [
				...
				// -------------------------
			{
				displayName: 'Input Binary Properties',
					...
					type: 'string', default: undefined,
					placeholder: 'e.g., data OR image1,image2',
					description:
					...
					},
				{
					...
					{ displayName: 'Width', name: 'imageWidth', type: 'number', default: undefined, placeholder: 'e.g., 1200', description: 'Set output width in pixels for resize/format actions. Use 0 to let H2I Engine (Pixlab) auto-calculate from height and aspect ratio. e.g. 1200.', displayOptions: { show: { resource: ['image'], operation: ['resize', 'format'] } } },
					{ displayName: 'Height', name: 'imageHeight', type: 'number', default: undefined, placeholder: 'e.g., 800', description: 'Set output height in pixels for resize/format actions. Use 0 to auto-calculate from width and aspect ratio. e.g. 800.', displayOptions: { show: { resource: ['image'], operation: ['resize', 'format'] } } },
				{
					displayName: 'Resize Mode',
					name: 'imageResizeMode',
					type: 'options',
					default: 'fit',
					description: 'Choose how the image is resized to the target Width/Height. fit preserves aspect ratio inside the box, cover fills and crops, pad fills with background to exact size, force stretches to exact size. e.g. fit.',
					options: [
						{ name: 'Fit inside', value: 'fit' },
						{ name: 'Fill and crop', value: 'cover' },
						{ name: 'Pad to exact size', value: 'pad' },
						{ name: 'Force exact size', value: 'force' },
					],
					displayOptions: { show: { resource: ['image'], operation: ['resize'] } },
				},
				{
					displayName: 'Enlarge Mode',
					name: 'imageEnlargeMode',
					type: 'options',
					default: 'automatic',
					description: 'Control upscale behavior for resize. Automatic omits enlarge and lets PixLab infer behavior, Enabled sends enlarge=true, Disabled sends enlarge=false.',
					options: [
				...
				{ displayName: 'Normalize Orientation', name: 'normalizeOrientation', type: 'boolean', default: false, description: 'Whether to normalize image orientation using EXIF metadata before processing. Enable for photos from phones/cameras that may appear rotated. e.g. enabled for mobile uploads.', displayOptions: { show: { resource: ['image'], operation: ['resize', 'crop', 'enhance', 'metadata'] } } },
					{ displayName: 'Crop X', name: 'cropX', type: 'number', default: undefined, placeholder: 'e.g., 0', description: 'Set crop start X coordinate in pixels from the left edge. Provide Crop Width and Crop Height so crop fields are applied. e.g. 120.', displayOptions: { show: { resource: ['image'], operation: ['crop'] } } },
					{ displayName: 'Crop Y', name: 'cropY', type: 'number', default: undefined, placeholder: 'e.g., 0', description: 'Set crop start Y coordinate in pixels from the top edge. Provide Crop Width and Crop Height so crop fields are applied. e.g. 80.', displayOptions: { show: { resource: ['image'], operation: ['crop'] } } },
					{ displayName: 'Crop Width', name: 'cropWidth', type: 'number', default: undefined, placeholder: 'e.g., 800', description: 'Set the crop box width in pixels. Use together with Crop X, Crop Y, and Crop Height for deterministic cropping. e.g. 600.', displayOptions: { show: { resource: ['image'], operation: ['crop'] } } },
					{ displayName: 'Crop Height', name: 'cropHeight', type: 'number', default: undefined, placeholder: 'e.g., 600', description: 'Set the crop box height in pixels. Use together with Crop X, Crop Y, and Crop Width for deterministic cropping. e.g. 400.', displayOptions: { show: { resource: ['image'], operation: ['crop'] } } },
					{ displayName: 'Background Color', name: 'backgroundColor', type: 'string', default: undefined, placeholder: 'e.g., #ffffff', description: 'Set the background fill color for actions that support it, using hex or CSS-like color values. Invalid color strings can cause invalid_parameter errors. e.g. #ffffff.', displayOptions: { show: { resource: ['image'], operation: ['crop', 'compress', 'background'] } } },
					{ displayName: 'Rotate (degrees)', name: 'rotate', type: 'number', default: undefined, placeholder: 'e.g., 90', description: 'Set rotation in degrees for transform actions. Use positive or negative values as needed by your layout. e.g. 90.', displayOptions: { show: { resource: ['image'], operation: ['transform'] } } },
				...
				{ displayName: 'Color Space', name: 'colorSpace', type: 'options', default: 'srgb', description: 'Choose output color space for transform/compress actions. cmyk may be rejected depending on H2I Engine (Pixlab) build configuration. e.g. srgb.', options: [{ name: 'sRGB', value: 'srgb' }, { name: 'Grayscale', value: 'grayscale' }, { name: 'CMYK', value: 'cmyk' }], displayOptions: { show: { resource: ['image'], operation: ['transform', 'compress'] } } },
					{ displayName: 'Target Size (KB)', name: 'targetSizeKB', type: 'number', default: undefined, placeholder: 'e.g., 200', description: 'Set target file size in kilobytes for compress action guidance. Leave 0 to skip target-size tuning. e.g. 250.', displayOptions: { show: { resource: ['image'], operation: ['compress'] } } },
					{ displayName: 'Quality', name: 'quality', type: 'number', default: undefined, placeholder: 'e.g., 80', description: 'Set encoder quality used by compression/output format paths. Higher values keep detail but produce larger files. e.g. 82.', displayOptions: { show: { resource: ['image'], operation: ['compress'] } } },
				{
					...
					{ displayName: 'Blur', name: 'blur', type: 'number', default: undefined, placeholder: 'e.g., 2', description: 'Set blur strength for enhance action; the API clamps to valid range. Use 0 to skip blurring. e.g. 2.', displayOptions: { show: { resource: ['image'], operation: ['enhance'] } } },
					{ displayName: 'Sharpen', name: 'sharpen', type: 'number', default: undefined, placeholder: 'e.g., 1', description: 'Set sharpen amount for enhance action; the API clamps to valid range. Use 0 to skip sharpening. e.g. 1.5.', displayOptions: { show: { resource: ['image'], operation: ['enhance'] } } },
				...
				{ displayName: 'Sepia', name: 'sepia', type: 'boolean', default: false, description: 'Whether to apply a sepia color effect in enhance action. Use with moderate contrast/brightness for vintage look. e.g. enabled for retro style.', displayOptions: { show: { resource: ['image'], operation: ['enhance'] } } },
					{ displayName: 'Brightness', name: 'brightness', type: 'number', default: undefined, placeholder: 'e.g., 1.1', description: 'Adjust image brightness for the enhance action. Leave 0 to omit this field and let the API default apply. e.g. 0.9.', displayOptions: { show: { resource: ['image'], operation: ['enhance'] } } },
					{ displayName: 'Contrast', name: 'contrast', type: 'number', default: undefined, placeholder: 'e.g., 1.1', description: 'Adjust image contrast for the enhance action. Leave 0 to omit this field and let the API default apply. e.g. 1.1.', displayOptions: { show: { resource: ['image'], operation: ['enhance'] } } },
					{ displayName: 'Saturation', name: 'saturation', type: 'number', default: undefined, placeholder: 'e.g., 1.1', description: 'Adjust image saturation for the enhance action. Leave 0 to omit this field and let the API default apply. e.g. 1.2.', displayOptions: { show: { resource: ['image'], operation: ['enhance'] } } },
					{ displayName: 'Pad', name: 'pad', type: 'number', default: undefined, placeholder: 'e.g., 20', description: 'Set uniform padding (pixels) applied to all sides when padding/frame is used. Side-specific values can override this in padding mode. e.g. 24.', displayOptions: { show: { resource: ['image'], operation: ['padding', 'frame'] } } },
					{ displayName: 'Pad Top', name: 'padTop', type: 'number', default: undefined, placeholder: 'e.g., 20', description: 'Set top padding in pixels for padding action. Use with other side fields for asymmetric spacing. e.g. 10.', displayOptions: { show: { resource: ['image'], operation: ['padding'] } } },
					{ displayName: 'Pad Right', name: 'padRight', type: 'number', default: undefined, placeholder: 'e.g., 20', description: 'Set right padding in pixels for padding action. Use with other side fields for asymmetric spacing. e.g. 20.', displayOptions: { show: { resource: ['image'], operation: ['padding'] } } },
					{ displayName: 'Pad Bottom', name: 'padBottom', type: 'number', default: undefined, placeholder: 'e.g., 20', description: 'Set bottom padding in pixels for padding action. Use with other side fields for asymmetric spacing. e.g. 10.', displayOptions: { show: { resource: ['image'], operation: ['padding'] } } },
					{ displayName: 'Pad Left', name: 'padLeft', type: 'number', default: undefined, placeholder: 'e.g., 20', description: 'Set left padding in pixels for padding action. Use with other side fields for asymmetric spacing. e.g. 20.', displayOptions: { show: { resource: ['image'], operation: ['padding'] } } },
					{ displayName: 'Pad Color', name: 'padColor', type: 'string', default: undefined, placeholder: 'e.g., #ffffff', description: 'Set pad fill color using hex/CSS-like syntax when padding or framing is applied. Leave empty to let API defaults apply where supported. e.g. #ffffff.', displayOptions: { show: { resource: ['image'], operation: ['padding', 'frame', 'background'] } } },
					{ displayName: 'Border', name: 'border', type: 'number', default: undefined, placeholder: 'e.g., 4', description: 'Set border thickness in pixels for frame action. Use 0 to remove border. e.g. 4.', displayOptions: { show: { resource: ['image'], operation: ['frame'] } } },
					{ displayName: 'Border Color', name: 'borderColor', type: 'string', default: undefined, placeholder: 'e.g., #000000', description: 'Set border color for frame action using hex/CSS-like color syntax. e.g. #000000.', displayOptions: { show: { resource: ['image'], operation: ['frame'] } } },
					{ displayName: 'Border Radius', name: 'borderRadius', type: 'number', default: undefined, placeholder: 'e.g., 12', description: 'Set border radius in pixels for rounded corners on frame/background actions. Use 0 for square corners. e.g. 12.', displayOptions: { show: { resource: ['image'], operation: ['padding', 'background'] } } },
					{ displayName: 'Background Blur', name: 'backgroundBlur', type: 'number', default: undefined, placeholder: 'e.g., 12', description: 'Set background blur strength for background action; API clamps large values. Use 0 to disable blur. e.g. 15.', displayOptions: { show: { resource: ['image'], operation: ['background'] } } },
					{ displayName: 'Watermark Text', name: 'watermarkText', type: 'string', default: undefined, placeholder: 'e.g., Sample watermark', description: 'Enter watermark text to overlay for image watermark action. Leave empty if you are using only an image watermark file. e.g. Confidential.', displayOptions: { show: { resource: ['image'], operation: ['watermark'] } } },
					{ displayName: 'Watermark Font Size', name: 'watermarkFontSize', type: 'number', default: undefined, placeholder: 'e.g., 32', description: 'Set text watermark font size in pixels. Increase for high-resolution images. e.g. 24.', displayOptions: { show: { resource: ['image'], operation: ['watermark'] } } },
					{ displayName: 'Watermark Color', name: 'watermarkColor', type: 'string', default: undefined, placeholder: 'e.g., #ffffff', description: 'Set text watermark color using hex/CSS-like syntax. e.g. #000000.', displayOptions: { show: { resource: ['image'], operation: ['watermark'] } } },
					{ displayName: 'Watermark Opacity', name: 'watermarkOpacity', type: 'number', default: undefined, placeholder: 'e.g., 0.35', description: 'Set text/image watermark opacity from 0 (transparent) to 1 (solid). Use lower values for subtle marks. e.g. 0.35.', displayOptions: { show: { resource: ['image'], operation: ['watermark'] } } },
				{
					...
					{ displayName: 'Watermark Margin', name: 'watermarkMargin', type: 'number', default: undefined, placeholder: 'e.g., 24', description: 'Set spacing around watermark placement in pixels. Increase to pull marks away from edges. e.g. 16.', displayOptions: { show: { resource: ['image'], operation: ['watermark'] } } },
					{ displayName: 'Watermark Scale', name: 'watermarkScale', type: 'number', default: undefined, placeholder: 'e.g., 0.25', description: 'Set watermark scaling multiplier for image watermark action. Use 1 for original size and adjust cautiously for readability. e.g. 1.2.', displayOptions: { show: { resource: ['image'], operation: ['watermark'] } } },
				{
					displayName: 'Watermark Image Binary Property',
					name: 'watermarkImageBinaryProp',
					type: 'string', default: undefined,
					placeholder: 'e.g., watermarkImage',
				...
				{
					displayName: 'Resize Mode',
					name: 'multitaskResizeMode',
					type: 'options',
					default: 'fit',
					description: 'Choose how resize behaves inside multitask. fit preserves aspect ratio inside the box, cover fills and crops, pad adds background to exact size, force stretches to exact size.',
					options: [
						{ name: 'Fit inside', value: 'fit' },
						{ name: 'Fill and crop', value: 'cover' },
						{ name: 'Pad to exact size', value: 'pad' },
				...
				{
					displayName: 'Enlarge Mode',
					name: 'multitaskEnlargeMode',
					type: 'options',
					default: 'automatic',
					description: 'Control upscale behavior for multitask resize. Automatic omits enlarge and lets PixLab infer behavior, Enabled sends enlarge=true, Disabled sends enlarge=false.',
					options: [
						{ name: 'Automatic', value: 'automatic' },
						{ name: 'Enabled', value: 'enabled' },
						{ name: 'Disabled', value: 'disabled' },
				...
				},
				{
				...
				},
					{
						...
						name: 'watermarkText',
						type: 'string', default: undefined,
						placeholder: 'e.g., Sample watermark',
				...
				// -------------------------
				{
					...
					name: 'pdfBinaryProps',
					type: 'string', default: undefined,
					placeholder: 'e.g., data OR pdf1,pdf2',
					description:
				...
				},
				{
					...
					{ displayName: 'Width', name: 'pdfWidth', type: 'number', default: undefined, placeholder: 'e.g., 1200', description: 'Set optional output width in pixels for PDF page image conversion. Leave 0 to keep API/default sizing. e.g. 1200.', displayOptions: { show: { resource: ['pdf'], operation: ['to-images'] } } },
					{ displayName: 'Height', name: 'pdfHeight', type: 'number', default: undefined, placeholder: 'e.g., 1600', description: 'Set optional output height in pixels for PDF page image conversion. Leave 0 to keep API/default sizing. e.g. 1600.', displayOptions: { show: { resource: ['pdf'], operation: ['to-images'] } } },
					{ displayName: 'Quality', name: 'pdfQuality', type: 'number', default: undefined, placeholder: 'e.g., 90', description: 'Set optional output quality for PDF page image conversion when the selected format supports quality controls. Leave 0 to keep API/default quality behavior. e.g. 82.', displayOptions: { show: { resource: ['pdf'], operation: ['to-images'] } } },
					{ displayName: 'DPI', name: 'dpi', type: 'number', default: undefined, placeholder: 'e.g., 150', description: 'Set render density (DPI) for PDF page rasterization. This is sent as density in the request. Higher DPI improves detail but increases processing time and output size. e.g. 300.', displayOptions: { show: { resource: ['pdf'], operation: ['to-images'] } } },
				...
				{ displayName: 'Watermark Text', name: 'watermarkText', type: 'string', default: undefined, placeholder: 'e.g., Internal use only', description: 'Enter optional text watermark for PDF watermark action. Leave empty when only image watermark is needed. e.g. Internal use only.', displayOptions: { show: { resource: ['pdf'], operation: ['watermark'] } } },
					{ displayName: 'Watermark Opacity', name: 'watermarkOpacity', type: 'number', default: undefined, placeholder: 'e.g., 0.2', description: 'Set watermark opacity from 0 to 1 for PDF watermark action. Lower values create subtler overlays. e.g. 0.25.', displayOptions: { show: { resource: ['pdf'], operation: ['watermark'] } } },
				{
					displayName: 'Watermark Position',
					name: 'watermarkPosition',
					type: 'options',
					...
					{ displayName: 'Watermark Margin', name: 'watermarkMargin', type: 'number', default: undefined, placeholder: 'e.g., 24', description: 'Set spacing around the PDF watermark placement in pixels. Increase to keep the mark away from page edges. e.g. 12.', displayOptions: { show: { resource: ['pdf'], operation: ['watermark'] } } },
					{
					...
					{ displayName: 'Watermark Font Size', name: 'watermarkFontSize', type: 'number', default: undefined, placeholder: 'e.g., 32', description: 'Set text watermark font size for PDF watermark action. Increase for high-resolution or large-page documents. e.g. 24.', displayOptions: { show: { resource: ['pdf'], operation: ['watermark'] } } },
				{ displayName: 'Watermark Color', name: 'watermarkColor', type: 'string', default: undefined, placeholder: 'e.g., #000000', description: 'Set text watermark color for PDF watermark action using hex or CSS-like color values. e.g. #000000.', displayOptions: { show: { resource: ['pdf'], operation: ['watermark'] } } },
					{ displayName: 'Watermark Scale', name: 'watermarkScale', type: 'number', default: undefined, placeholder: 'e.g., 0.25', description: 'Set watermark scaling multiplier for PDF/image watermark operations. Use 1 for default size. e.g. 0.8.', displayOptions: { show: { resource: ['pdf'], operation: ['watermark'] } } },
				{
					displayName: 'Watermark Image Binary Property',
					name: 'watermarkImageBinaryProp',
					type: 'string', default: undefined,
				...
				{ displayName: 'Degrees', name: 'degrees', type: 'options', default: 90, description: 'Select clockwise rotation angle applied to selected PDF pages. Use together with Pages field to target specific pages. e.g. 90.', options: [{ name: '90', value: 90 }, { name: '180', value: 180 }, { name: '270', value: 270 }], displayOptions: { show: { resource: ['pdf'], operation: ['rotate'] } } },
					{ displayName: 'Title', name: 'title', type: 'string', default: undefined, placeholder: 'e.g., Project Report', description: 'Set PDF Title metadata value in metadata action. Leave empty to keep existing title unless clean is enabled. e.g. Q1 Report.', displayOptions: { show: { resource: ['pdf'], operation: ['metadata'] } } },
					{ displayName: 'Author', name: 'author', type: 'string', default: undefined, placeholder: 'e.g., John Doe', description: 'Set PDF Author metadata value in metadata action. e.g. Finance Team.', displayOptions: { show: { resource: ['pdf'], operation: ['metadata'] } } },
					{ displayName: 'Subject', name: 'subject', type: 'string', default: undefined, placeholder: 'e.g., Monthly Summary', description: 'Set PDF Subject metadata value in metadata action. e.g. Quarterly performance.', displayOptions: { show: { resource: ['pdf'], operation: ['metadata'] } } },
					{ displayName: 'Keywords', name: 'keywords', type: 'string', default: undefined, placeholder: 'e.g., report,monthly,internal', description: 'Set PDF Keywords metadata as a comma-separated text string. e.g. finance,quarterly,kpi.', displayOptions: { show: { resource: ['pdf'], operation: ['metadata'] } } },
					{ displayName: 'Creator', name: 'creator', type: 'string', default: undefined, placeholder: 'e.g., n8n', description: 'Set PDF Creator metadata value in metadata action. e.g. n8n workflow.', displayOptions: { show: { resource: ['pdf'], operation: ['metadata'] } } },
					{ displayName: 'Producer', name: 'producer', type: 'string', default: undefined, placeholder: 'e.g., Davix H2I', description: 'Set PDF Producer metadata value in metadata action. e.g. Davix H2I Engine (Pixlab) API.', displayOptions: { show: { resource: ['pdf'], operation: ['metadata'] } } },
				...
				{ displayName: 'Order (CSV or JSON Array)', name: 'order', type: 'string', default: undefined, placeholder: 'e.g., 2,1,3', description: 'Enter page order as CSV or JSON array of positive page numbers. The node converts JSON arrays to CSV before sending to the API. e.g. 2,1,3.', displayOptions: { show: { resource: ['pdf'], operation: ['reorder'] } } },
				...
				{ displayName: 'User Password', name: 'userPassword', type: 'string', default: undefined, typeOptions: { password: true }, placeholder: 'e.g., your user password', description: 'Set required user password for PDF encryption. Use a strong value and store it securely outside workflow logs. e.g. user-pass-2026.', displayOptions: { show: { resource: ['pdf'], operation: ['encrypt'] } } },
				{ displayName: 'Owner Password', name: 'ownerPassword', type: 'string', default: undefined, typeOptions: { password: true }, placeholder: 'e.g., your owner password', description: 'Set optional owner password for PDF encryption; if blank, API may default to user password. Use a separate value for admin permissions. e.g. owner-pass-2026.', displayOptions: { show: { resource: ['pdf'], operation: ['encrypt'] } } },
				{ displayName: 'Password', name: 'password', type: 'string', default: undefined, typeOptions: { password: true }, placeholder: 'e.g., your password', description: 'Enter the current password required to decrypt a protected PDF. Decryption fails if this value is incorrect. e.g. current-doc-pass.', displayOptions: { show: { resource: ['pdf'], operation: ['decrypt'] } } },
				...
				// -------------------------
				{
					...
					type: 'string', default: undefined,
					placeholder: 'e.g., data OR img1,img2',
			...
			try {
				...
				if (resource === 'image') {
					...
					switch (action) {
						...
						case 'resize':
							...
							setNumber('width', this.getNodeParameter('imageWidth', itemIndex) as number);
							setNumber('height', this.getNodeParameter('imageHeight', itemIndex) as number);
							setString('resizeMode', this.getNodeParameter('imageResizeMode', itemIndex) as string);
							{
								const enlargeMode = this.getNodeParameter('imageEnlargeMode', itemIndex) as string;
								if (enlargeMode === 'enabled') {
									setBool('enlarge', true);
								} else if (enlargeMode === 'disabled') {
									setBool('enlarge', false);
								}
							}
									...
									case 'resize':
										{
											setNumberOnce('width', this.getNodeParameter('width', itemIndex) as number);
											setNumberOnce('height', this.getNodeParameter('height', itemIndex) as number);
											setStringOnce('resizeMode', this.getNodeParameter('multitaskResizeMode', itemIndex) as string);
											const enlargeMode = this.getNodeParameter('multitaskEnlargeMode', itemIndex) as string;
											if (enlargeMode === 'enabled') {
												setBoolOnce('enlarge', true);
											} else if (enlargeMode === 'disabled') {
												setBoolOnce('enlarge', false);
											}
				...
				if (resource === 'pdf') {
					...
					if (action === 'to-images') {
						...
						setString('format', this.getNodeParameter('toFormat', itemIndex) as string);
						...
						setNumber('quality', this.getNodeParameter('pdfQuality', itemIndex) as number);
						setNumber('density', this.getNodeParameter('dpi', itemIndex) as number);
					}

					if (action === 'extract-images') {
						setString('pages', this.getNodeParameter('pages', itemIndex) as string);
						setString('format', this.getNodeParameter('extractImageFormat', itemIndex) as string);
					}
